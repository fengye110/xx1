...
function gen-sysfs() {
        ...
        dh='include/linux/sysfs.h'
        gen NEED_SYSFS_CREATE_GROUPS if fun sysfs_create_groups absent in "$dh"
        gen NEED_SYSFS_REMOVE_GROUPS if fun sysfs_remove_groups absent in "$dh"
}

function gen-aux() {
	ah='include/linux/mucse_auxiliary_bus.h'
