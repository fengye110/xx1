...
cmd_depmod = /sbin/depmod $(if ${SYSTEM_MAP_FILE},-F ${SYSTEM_MAP_FILE}) \
...
$(if $(shell \
    ...
    KSRC=${KSRC} OUT=${src}/compat/kcompat_generated_defs.h CONFFILE=${CONFIG_FILE} \
    bash ${src}/compat/kcompat-generator.sh && echo ok), , $(error kcompat-generator.sh failed))

KCOMPAT_DEFINITIONS := $(shell ${CC} ${EXTRA_CFLAGS} -E -dM \
                                     -I${KOBJ}/include \
                                     -I${KOBJ}/include/generated/uapi \
                                     ${src}/compat/kcompat_defs.h | awk '{ print $$2 }')
...
AUX_BUS_HEADERS ?= linux/mucse_auxiliary_bus.h compat/auxiliary_compat.h compat/kcompat_generated_defs.h
ifeq (${NEED_AUX_BUS},2)
define auxiliary_post_install
	install -D -m 644 Module.symvers ${INSTALL_MOD_PATH}/lib/modules/${KVER}/extern-symvers/mucse_auxiliary.symvers
	install -d ${INSTALL_MOD_PATH}/lib/modules/${KVER}/${INSTALL_AUX_DIR}
	mv -f ${INSTALL_MOD_PATH}/lib/modules/${KVER}/${INSTALL_MOD_DIR}/mucse_auxiliary.ko* \
	...
	install -D -m 644 mce_idc.h          -t ${INSTALL_MOD_PATH}/${KSRC}/include/linux
endef
else
...
define auxiliary_post_uninstall
	rm -f ${INSTALL_MOD_PATH}/lib/modules/${KVER}/extern-symvers/mucse_auxiliary.symvers
	rm -f ${INSTALL_MOD_PATH}/lib/modules/${KVER}/${INSTALL_AUX_DIR}/mucse_auxiliary.ko*
	rm -f ${INSTALL_MOD_PATH}/${KSRC}/include/linux/mucse_auxiliary_bus.h
	rm -f ${INSTALL_MOD_PATH}/${KSRC}/include/linux/compat/auxiliary_compat.h
	rm -f ${INSTALL_MOD_PATH}/${KSRC}/include/linux/compat/kcompat_generated_defs.h
	rm -f ${INSTALL_MOD_PATH}/${KSRC}/include/linux/mce_idc.h
endef
else
...
ifeq (${NEED_AUX_BUS},2)
EXTRA_CFLAGS += -DUSE_MUCSE_AUX_BUS
endif
######################
# Kernel Build Macro #
######################
